...
export default async function FacultyExamsPage() {
  if (!hasPublicSupabaseEnv()) return <Fallback title="Faculty Exams" />;
  ...
  const [{ data: courses }, { data: exams }] = await Promise.all([
    ...
    supabase.from("exams").select("id, course_id, semester_no, name, exam_type, max_marks, exam_date, courses(code)").eq("institution_id", institutionId).order("exam_date", { ascending: false })
  ]);

  return (
    <AdminEntityManager
      title="Faculty Exam Setup"
      description="Create and manage assessments directly from the faculty workspace."
      endpoint="/api/faculty/exams"
      fields={[
        { name: "courseId", label: "Course", type: "select", options: (courses ?? []).map((course) => ({ label: `${course.name} (${course.code})`, value: course.id })) },
        { name: "semesterNo", label: "Semester", type: "number" },
        { name: "name", label: "Exam Name" },
        { name: "examType", label: "Exam Type", type: "select", options: ["internal", "external", "practical", "assignment"].map((item) => ({ label: item, value: item })) },
